...

{
...

    {
        ...
        //Hjælpemetode til UpdatePerson 

        private bool ExistsPersonWithID(int id)

        {
            ...
            //var sql = $"SELECT navn as personnavn, tlf as persontlf, email as personemail FROM person WHERE person_id = {id}"; 

            var sql2 = $"SELECT 1 FROM person WHERE EXISTS(SELECT 1 FROM person where person_id = {id})";

            var person = db.connection.Query(sql2);
        ...
        /*public List<Person> GetMaxIDPerson() 

        { 

            Console.WriteLine("get Max ID for personer repository"); 

            var sql = "SELECT MAX (person_id as personid) AS nyesteperson FROM person"; 
        ...
        private bool ExistsVagtWithID(int id)

        {
            ...
            var sql2 = $"SELECT 1 FROM vagter WHERE EXISTS(SELECT 1 FROM vagter where vagt_id = {id})";

            var vagt = db.connection.Query(sql2);
